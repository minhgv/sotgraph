# Agent Rules & Protocols (sotgraph SSOT v0.3.6)
...
# sotgraph Single Source of Truth Protocols & Rules for Agents (SSOT v0.3.6)
...
2. **Modes**: `auto` (default — reconcile only when stale) | `force` (always) | `off` (skip). CLI: `--reconcile auto|force|off` (diff-impact: `--auto-reconcile`/`--no-auto-reconcile`, default on). MCP: `auto_reconcile` parameter on the 8 gated tools. `jit_mode` config (`SOT_JIT_MODE`): `async` (default — two-tier healing, below) | `blocking` (inline reconcile before answering). `force` and gate paths always reconcile inline.
3. **Two-tier healing (async)**: on a stale probe, a *fast tier* first runs builtin per-file reconcile on the drifted paths inline (~0.2–0.5s for small deltas) — under newest-wins union ownership those rows outrank the older engine snapshot, so the query sees FRESH content immediately. A debounced background deep reconcile (engine re-index + snapshot publish) then restores the deep graph — spawned only when due (publish age ≥ `SOT_CBM_MIN_INTERVAL_S`, default 300s; always when files were deleted, since only a republish clears engine ghost rows; skipped when the delta exceeds `SOT_FAST_TIER_MAX`, default 64 paths).
4. **Engine daemon**: `reconcile` prefers a per-repo persistent engine session (`.sot/cbm/daemon.sock`, auto-started, 15-min idle TTL) over a cold process spawn — a warm `index_repository` call is ~3.5x faster (~4.5s vs ~15.7s). The daemon is an optimization only: any failure falls back to the cold spawn.
5. **Disclosure**: gated MCP responses carry a `graph_freshness` envelope (probe counts, reconcile `refreshed|background|skipped_fresh|failed`, `serving: fresh|stale`, `background: spawned|debounced|already_running|spawn_failed`). CLI prints a `↻ JIT reconcile: ...` notice on stderr.
6. **Never blocks**: if reconcile fails (DB lock, parse error), the query still answers from the stale graph with `status: failed` disclosed — check the envelope before trusting freshness-sensitive verdicts.
7. **Known blind spot**: same-size edits within the same millisecond skip the probe hash; the assurance layer still hash-checks cited paths post-query. Explicit `sotgraph reconcile` (section 5) remains the primary heal after bulk changes.
