...
2. **Modes**: `auto` (default — reconcile only when stale) | `force` (always) | `off` (skip). CLI: `--reconcile auto|force|off` (diff-impact: `--auto-reconcile`/`--no-auto-reconcile`, default on). MCP: `auto_reconcile` parameter on the 8 gated tools. `jit_mode` config (`SOT_JIT_MODE`): `async` (default — stale probe spawns a detached `sotgraph reconcile` behind a pid-lock debounce and answers from the current snapshot) | `blocking` (inline reconcile before answering). `force` and gate paths always reconcile inline.
3. **Disclosure**: gated MCP responses carry a `graph_freshness` envelope (probe counts for modified/deleted/unindexed files, reconcile `performed|skipped|failed|background` status; `serving: "stale"` when async). CLI prints a `↻ JIT reconcile: ...` notice on stderr when a reconcile ran or spawned.
