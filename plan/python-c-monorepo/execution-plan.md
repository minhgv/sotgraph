...
- WP0.3 Spike chạy sống an toàn trên scratch repo (lifecycle, cancellation, artifact side effects). **[UNBLOCKED — isolation protocol xác định từ source tag `46ae198f` (`provenance-upstream.md` mục 5); spike còn phải chạy]**
- WP0.4 Audit license/packaging/source availability của candidate `3c7427e`. **[DONE — MIT + notices đầy đủ; upstream pin chuẩn = `v0.10.8` = `46ae198f`; distribution repo = `DeusData/codebase-memory-mcp`]**
...
- WP0.7 Tool-schema inventory + operation compatibility matrix + runtime capability report (historical/mock vs live-unknown, blocker ledger). **[ĐÃ LÀM — live cột UNKNOWN]**
...
**Trạng thái G0 hiện tại: UNBLOCKED (2026-09-05 continuation).** WP0.2 identity ĐÃ ĐÓNG bằng release-manifest binding (distribution repo `DeusData/codebase-memory-mcp`, tag `v0.10.8` = `46ae198f`, checksums.txt digest-verified, byte-identity installed↔release) + WP0.4 license/source DONE. User authorization 2026-09-05: external third-party attestation KHÔNG bắt buộc — trusted evidence gồm release-manifest binding hoặc controlled reproducible build receipt từ source pin (đúng mục 7 tài liệu gốc "manifest phát hành tin cậy"). State-dir toàn cục đã xác định từ source ⇒ scratch-isolation protocol sẵn sàng (`evidence/provenance-upstream.md` mục 5). Còn lại: WP0.3 live spike để xóa giả định kill CLI = cancel worker.
