...
owner-instance sync semantics, NO recovery API (frozen namespaces; recovery =
fresh generation), digest+generation namespaces, and zero writes into the
source repo.
"""
...
def test_readonly_status_never_creates(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    before = dict(os.environ)
    status = _profile(root, repo).status()
    assert status["initialized"] is False
    assert status["state"] == "UNINITIALIZED"
    assert not os.path.exists(root)  # pure read created nothing at all
    assert os.environ == before


def test_initialize_creates_private_dirs_and_ui_seed(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
...
def test_reuse_exact_identity_ready(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
...
def test_identity_mismatch_refused_manifest_intact(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    profile = _profile(root, repo)
    profile.initialize()
...
def test_preexisting_unowned_namespace_refused_no_overwrite(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    profile = _profile(root, repo)
...
def test_seed_tamper_env_refuses_with_no_writes(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    profile = _profile(root, repo)
    profile.initialize()
    seed = profile.paths["cache"] / "config.json"
...
def test_launch_env_allowlist_isolated_no_global_mutation(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    profile = _profile(root, repo)
    profile.initialize()
    before = dict(os.environ)
...
def test_symlinks_rejected(tmp_path) -> None:
    ...
    good = _profile(tmp_path / "root2", repo)
    good.initialize()
...
def test_no_recovery_api_and_unknown_state_quarantines(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    profile = _profile(root, repo)
    profile.initialize()
...
def test_begin_sync_owner_only_environment(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    ...
    assert owner.environment()


@pytest.mark.parametrize("confirmed,success", [(True, False), (False, True), (False, False)])
def test_unconfirmed_complete_quarantines(tmp_path, confirmed, success) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    profile = _profile(root, repo)
    profile.initialize()
    profile.begin_sync()
...
def test_two_digests_coexist_independent_namespaces(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    pa, pb = _profile(root, repo, digest="a" * 64), _profile(root, repo, digest="b" * 64)
    # 32-hex collision-resistant composite (NOT an absolute guarantee; the
    # manifest identity check is what actually gates every use).
    assert re.search(r"managed-initial-[0-9a-f]{32}-[0-9a-f]{32}$", pa.namespace.name)
...
def test_fresh_generation_same_artifact_leaves_old_frozen(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
...
def test_generation_must_be_identifier(tmp_path) -> None:
    repo = _mk_repo(tmp_path / "w")
    with pytest.raises(ProfileRejected):
        _profile(tmp_path / "r", repo, generation="../evil")
    with pytest.raises(ProfileRejected):
        _profile(tmp_path / "r", repo, generation="")
    with pytest.raises(ProfileRejected):
        _profile(tmp_path / "r", repo, generation="x" * 65)


def test_existing_root_mode_unchanged_on_rejection(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
...
def test_source_repo_and_notes_untouched(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    def snapshot(base):
...
def test_independent_repo_namespaces(tmp_path) -> None:
    root = tmp_path / "root"
...
def test_root_must_be_absolute_and_outside_repo(tmp_path) -> None:
    repo = _mk_repo(tmp_path / "w")
    with pytest.raises(ProfileRejected):
...
def test_concurrent_initialize_serialized_by_write_lock(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
...
def test_lock_contention_fails_closed(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    profile = _profile(root, repo)
    profile.initialize()
...
def test_sync_thread_ownership(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    profile = _profile(root, repo)
    profile.initialize()
...
def test_tamper_persisted_by_mutations_never_ready(tmp_path) -> None:
    root, repo = tmp_path / "root", _mk_repo(tmp_path / "w")
    profile = _profile(root, repo)
    profile.initialize()
